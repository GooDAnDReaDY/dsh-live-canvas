...
## 🧭 Совместимость с боковыми панелями DSH

`@goodandready/dsh-live-canvas` бесшовно адаптируется под любое окружение DSH без внешних зависимостей:

| Конфигурация DSH | Поддерживаемая поверхность | Возможности |
|---|---|---|
| **DSH 0.1.5-alpha.1+ (Нативный)** | Встроенная правая панель (`sidebarRightTabs`, слот `sidebar.right.pane.tab`) | Полноценная вкладка Live Canvas + системный просмотрщик файлов `dsh-resource://file` (`.html`, `.jsx`, `.tsx`, `.svg`, `.mermaid`, `.md`). |
| **Legacy профили** | Интеграция с `dsh-better-sidebar` | Вкладка (`live-canvas`) и просмотрщик файлов (`live-canvas-viewer`). |
| **Оба сайдбара активны** | Бесшовное сосуществование | Детерминированные раздельные ID, отсутствие дублирующих монтирований и независимые функции очистки. |
| **Без сайдбаров (Минимальный)** | Автономная шторка и модальные окна | Полностью автономный slide-over холст по горячим клавишам (`Alt+C`) без необходимости в сторонних плагинах. |
...
- **🏠 Хаб проектов и недавних сессий (v0.2.14)**: Интерактивный дашборд с недавними сессиями и компонентами рабочей области (React, HTML, Vue, SVG, Markdown), отсортированными по времени, с живым поиском и открытием в один клик.
- **↕️ Интерактивный ресайз карточек в чате**: Плавное изменение высоты (от 160px до 800px) за нижний край, быстрые пресеты (`S` 200px, `M` 320px, `L` 480px) и полноширинный режим Wide View (`⤢`).
- **🔄 Мгновенное обновление холста**: Кнопка `🔄 Обновить` в панели управления и поддержка <kbd>F5</kbd> / <kbd>Ctrl+R</kbd> внутри фрейма без перезагрузки интерфейса DSH.
- **🛡️ Защитные схемы ответов инструментов**: Структурированные ответы об ошибках доступа (`code`, `allowedRoots`, `reason`) для автономного исправления действий агентом.
...
Те же значения редактируются на странице **Live Canvas Preview** в разделе плагинов. Под заголовком одна строка описания, ниже форма настроек. Сохранение записывает каждое поле через сервис настроек плагина. Пока сервис загружается, форма показывает состояние загрузки. Если сервис недоступен, форма показывает состояние недоступности, а не страницу ошибки. Пространство имён настроек — `dsh-live-canvas`; имя пакета не является допустимым идентификатором настроек.
...
plugins:
  "@goodandready/dsh-live-canvas":
    ...
    workspaceRoots: []            # Разрешённые корневые директории для безопасного доступа к файлам
```
...
| `workspaceRoots` | `string[]` | `[]` | Явно разрешённые корневые каталоги для просмотра файлов и сканирования. Защищает от path traversal за пределы разрешённых папок. |
...
## 🔒 Безопасность и изоляция (Sandboxing)

Live Canvas использует многоуровневую модель защиты:
- **Строгая изоляция путей (`resolveSafePath`)**: Любые операции чтения файлов через API и инструменты агента (`live_canvas_preview`, `/dsh-live-canvas/api/open-file`, `/dsh-live-canvas/api/workspace/files`) строго валидируются на нахождение внутри разрешённых каталогов (`workspaceRoots` или рабочий каталог процесса). Попытки чтения чувствительных системных файлов (например, `/etc/shadow`) или соседних директорий блокируются ошибкой `ERR_PATH_OUTSIDE_ROOTS` и статусом HTTP 403 Forbidden.
- **Защита от CSRF и проверка источника (`isTrustedRequest`)**: Все мутирующие эндпоинты требуют совпадения `Origin` / `Referer` с хостом DSH, проверки `Sec-Fetch-Site: same-origin` или локального loopback-вызова.
- **Content Security Policy в iframe**: Песочницы рендерятся в изолированных iframe с заголовками безопасности (`X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`) и строгой CSP.

---

## 🔄 Встроенный 1-Click апдейтер хоста

Live Canvas содержит стандартный механизм обновления DSH:
- **Автоматическая проверка обновлений**: Проверяет новые версии на `registry.npmjs.org` с корректной поддержкой semver prerelease.
- **UI в карточке плагина**: Отображает текущую версию, статус обновления, кнопку принудительной проверки и кнопку 1-click обновления.
- **Безопасная установка на хосте**: Запускает обновление профиля через пакетный менеджер хоста (`pnpm` / `dsh plugin add`), проверяя loopback и специальный заголовок `X-DSH-Update-Key`.
