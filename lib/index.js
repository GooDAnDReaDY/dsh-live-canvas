...
<body class="min-h-screen flex items-center justify-center p-6 select-none bg-zinc-950">
  <div class="max-w-md w-full text-center space-y-6">
    ...
    <div class="space-y-2">
      <h2 class="text-2xl font-bold tracking-tight text-white">Interactive Canvas</h2>
      <p class="text-sm text-zinc-400">
        Ask an agent in chat to create any UI (React, Tailwind, HTML, SVG, Mermaid), and it will automatically render here in real time.
      </p>
    </div>
    ...
    <div class="pt-4 border-t border-zinc-800/80 space-y-3">
      ...
      <div class="grid grid-cols-1 gap-2 text-left">
        <button onclick="loadDemo('calc')" class="p-3 rounded-xl bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-800 text-sm font-medium flex items-center gap-3 transition cursor-pointer">
          ...
          <div>
            <div class="text-zinc-200">React Calculator</div>
            <div class="text-xs text-zinc-400">Interactive calculator with Tailwind</div>
          </div>
        </button>
        <button onclick="loadDemo('dash')" class="p-3 rounded-xl bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-800 text-sm font-medium flex items-center gap-3 transition cursor-pointer">
          <span class="text-xl">📊</span>
          <div>
            <div class="text-zinc-200">SaaS Metrics Dashboard</div>
            <div class="text-xs text-zinc-400">KPI cards and analytics</div>
          </div>
        </button>
      </div>
    </div>
  ...
  <script>
    try {
      ...
      sse.onerror = (err) => {
        console.warn('[LiveCanvas] Welcome preview SSE disconnected:', err);
      };
    } catch (err) {
      console.warn('[LiveCanvas] Could not connect to event stream:', err);
    }
...
export default function Dashboard() {
  const kpis = [
    { title: "Monthly Recurring Revenue", value: "$48,250", change: "+14.2%", up: true },
    { title: "Active Users", value: "3,842", change: "+8.1%", up: true },
    { title: "Conversion Rate", value: "4.65%", change: "-0.4%", up: false },
    { title: "Average Order Value", value: "$124", change: "+5.3%", up: true }
  ];
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-8 space-y-6">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        <div>
          ...
          <p className="text-xs text-zinc-400">Overview of key metrics for the current month</p>
        </div>
        <div className="flex gap-2">
          <button className="px-3 py-1.5 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-xs rounded-lg">Export PDF</button>
          <button className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-lg">+ Report</button>
        </div>
      </div>
