    ...
    ctx.effect(() => {
      ...
      function buildProjectsHubHtml({ store, watcher }) {
        ...
        return '<!DOCTYPE html>\n<html lang="en" class="dark">\n<head>\n' +
          ...
          '    if (!canvasId) return;\n' +
          '    try { if (window.parent && window.parent !== window) { window.parent.postMessage({ type: "dlc_open_session", canvasId: canvasId }, "*"); } } catch {} \n' +
          '    window.location.href = "/dsh-live-canvas/sandbox/" + canvasId;\n' +
          '  }\n' +
          '  function openFile(filePath) {\n' +
          '    if (!filePath) return;\n' +
          '    try { if (window.parent && window.parent !== window) { window.parent.postMessage({ type: "dlc_open_file", filePath: filePath }, "*"); } } catch {} \n' +
          ...
          '          try { if (window.parent && window.parent !== window) { window.parent.postMessage({ type: "dlc_session_created", canvasId: data.canvasId }, "*"); } } catch {} \n' +
