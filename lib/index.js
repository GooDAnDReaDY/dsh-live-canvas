...
const API_ROUTE_METHODS = {
  '/dsh-live-canvas/api/preview': ['POST'],
  '/dsh-live-canvas/api/inspect': ['POST'],
  '/dsh-live-canvas/api/inspections': ['GET'],
  '/dsh-live-canvas/api/sessions': ['GET'],
  '/dsh-live-canvas/api/snapshots': ['GET'],
  '/dsh-live-canvas/api/mock': ['GET', 'POST'],
  '/dsh-live-canvas/api/workspace-files': ['GET'],
  '/dsh-live-canvas/api/open-file': ['POST'],
  '/dsh-live-canvas/api/save-content': ['POST'],
  '/dsh-live-canvas/api/save-classes': ['POST'],
  '/dsh-live-canvas/api/ai-prompt': ['POST'],
  '/dsh-live-canvas/api/standalone': ['GET'],
  '/dsh-live-canvas/api/logs': ['GET', 'POST'],
  '/dsh-live-canvas/api/annotate': ['POST'],
  '/dsh-live-canvas/api/annotations': ['GET'],
  '/dsh-live-canvas/api/controls': ['GET', 'POST'],
  '/dsh-live-canvas/api/templates': ['GET'],
  '/dsh-live-canvas/api/save-reorder': ['POST'],
  '/dsh-live-canvas/api/themes': ['GET'],
  '/dsh-live-canvas/api/motion': ['GET'],
  '/dsh-live-canvas/api/mocks': ['GET'],
  '/dsh-live-canvas/api/share-info': ['GET'],
  '/dsh-live-canvas/api/deploy': ['GET'],
  '/dsh-live-canvas/api/sound-presets': ['GET'],
  '/dsh-live-canvas/api/annotations/resolve': ['POST'],
  '/dsh-live-canvas/api/update': ['GET', 'POST']
};

export function getAllowedMethodsForApiPath(pathname) {
  if (API_ROUTE_METHODS[pathname]) return API_ROUTE_METHODS[pathname];
  if (pathname.startsWith('/dsh-live-canvas/api/export/')) return ['GET'];
  if (pathname.startsWith('/dsh-live-canvas/api/pack/')) return ['GET'];
  return null;
}
...
export function apply(ctx, config) {
  ...
  // Register Web Server Endpoints and Settings Policy for DSH 0.1.7+ (#142)
  // Suppress auto-generated settings form since dsh-live-canvas provides its own
    ...
    if (settings && typeof settings.configure === 'function') {
      try {
        ...
        if (typeof ctx.effect === 'function') {
          ...
          }, 'dsh-live-canvas: settings policy');
        }
      } catch {
        /* ignoreOptionalFailure: safe fallback if configure is already registered or unsupported */
      }
    }
  };
  if (ctx.settings) {
    configureSettings(ctx.settings);
  } else if (typeof ctx.inject === 'function') {
    ctx.inject(['settings'], (sctx) => configureSettings(sctx && sctx.settings));
  }

  ctx.inject(['webServer'], (wctx) => {
    ctx.effect(() => {
      // 0. Host-side One-Click Plugin Updater (canonical and legacy alias #144)
      ...
      const unregUpdaterAlias = registerPluginUpdater(wctx, {
        endpoint: '/dsh-live-canvas/api/update',
        packageName: '@goodandready/dsh-live-canvas',
        manifestUrl: new URL('../package.json', import.meta.url)
      });
      ...
      const unregApi = wctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const parseBody = (maxBytes = 25 * 1024 * 1024) => new Promise((resolve, reject) => {
            ...
            const onError = (err) => {
              if (settled) return;
              settled = true;
              reject(err || new Error('Request body stream error'));
            };
            req.on('data', chunk => {
              if (settled) return;
              body += chunk;
      ...
      return () => {
        ...
        if (typeof unregUpdaterAlias === 'function') unregUpdaterAlias();
