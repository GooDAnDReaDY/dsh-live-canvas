...
// Cordis plugin providing Live Preview Sandbox server, SSE hot-reload, DOM click inspector, telemetry logs, standalone export, visual annotations, component galleries, workspace file watcher, props controls, visual diffs, device matrix, AI mock data, and agent tools.
...
export const Config = Schema.object({
  ...
  enableFileWatcher: Schema.boolean().default(true).description('Automatically watch referenced files on disk for live preview sync'),
  ...
  <p>Session ID <code>${canvasId || 'empty'}</code> does not exist or has expired.</p>
      ...
      const unregDiff = wctx.webServer.register({
        ...
        handler: (req, res) => {
          ...
          if (!session) {
            res.writeHead(404, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end('<h1>Canvas session not found</h1>');
            return;
          }

          const urlObj = new URL(req.url, 'http://localhost');
          const snapId = urlObj.searchParams.get('snapshotId');
          const prevSnap = snapId ? store.getSnapshot(canvasId, snapId) : store.getLatestPreviousSnapshot(canvasId);

          const diffHtml = buildDiffWrapper(session, prevSnap, {
            title: `Visual Diff: ${session.title}`
          });
      ...
      // 4. Multi-Device Matrix Endpoint
      const unregMatrix = wctx.webServer.register({
        ...
        handler: (req, res) => {
          ...
          const matrixHtml = buildMatrixWrapper(session, {
            title: `Multi-Device Matrix: ${session.title}`
          });
          res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
          res.end(matrixHtml);
        }
      ...
      // 5. API Endpoints for REST queries
