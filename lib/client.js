...
`;
    ...
    function PluginCard(props) {
      ...
      const subscribe = React.useMemo(() => (callback) => {
        if (!scope || typeof scope.subscribe !== 'function') return () => {};
        try {
          const unsub = scope.subscribe(callback);
      ...
      const snapshot = (typeof React.useSyncExternalStore === 'function')
        ? React.useSyncExternalStore(subscribe, getSnapshot, getSnapshot)
        : (scope && typeof scope.getSnapshot === 'function' ? scope.getSnapshot() : { status: 'unavailable', value: {} });
      ...
      const checkUpdate = React.useCallback(async () => {
        ...
        try {
          const res = await fetch('/api/dsh-live-canvas/update?action=check').then(r => r.json());
          setUpdater(res);
        } catch (err) {
      ...
      const triggerUpdate = async () => {
        ...
        try {
          const res = await fetch('/api/dsh-live-canvas/update', {
            method: 'POST',
    ...
    module.exports.apply = function apply(ctx) {
      if (typeof ctx.effect === 'function') {
        ctx.effect(() => {
          try {
            const dispose = ctx.locale?.register ? ctx.locale.register(NS, i18n) : undefined;
            return () => {
              if (typeof dispose === 'function') dispose();
            };
          } catch (e) {
            console.warn('[dsh-live-canvas] locale register failed', e && e.message || e);
          }
        }, 'dsh-live-canvas: locale');
      } else {
        try {
