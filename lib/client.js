...
.dlc-code-drawer { width:420px; background:var(--dsw-alias-bg-layer-3, #18181b); border-left:1px solid var(--dsw-alias-border-l2, #3f3f46); display:flex; flex-direction:column; z-index:20; box-shadow:-4px 0 20px rgba(0,0,0,0.3); }
.dlc-code-drawer-head { padding:8px 12px; border-bottom:1px solid var(--dsw-alias-border-l2, #3f3f46); display:flex; align-items:center; justify-content:space-between; }
.dlc-code-textarea { flex:1; padding:12px; background:var(--dsw-alias-bg-layer-1, #09090b); color:var(--dsw-alias-label-primary, #e4e4e7); font-family:ui-monospace, 'Cascadia Code', monospace; font-size:12px; line-height:1.6; resize:none; border:none; outline:none; }
.dlc-code-drawer-foot { padding:6px 12px; border-top:1px solid var(--dsw-alias-border-l2, #3f3f46); display:flex; align-items:center; justify-content:space-between; font-size:11px; color:var(--dsw-alias-label-secondary, #71717a); }
.dlc-blocks-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.7); backdrop-filter:blur(4px); z-index:50; display:flex; align-items:center; justify-content:center; padding:16px; }
.dlc-blocks-modal { background:var(--dsw-alias-bg-layer-3, #18181b); border:1px solid var(--dsw-alias-border-l2, #3f3f46); border-radius:16px; max-width:640px; width:100%; padding:20px; max-height:85vh; display:flex; flex-direction:column; box-shadow:0 20px 50px rgba(0,0,0,0.5); }
.dlc-blocks-list { flex:1; overflow-y:auto; }
.dlc-block-card { padding:12px; border-radius:12px; background:var(--dsw-alias-bg-layer-1, #09090b); border:1px solid var(--dsw-alias-border-l2, #3f3f46); margin-bottom:8px; display:flex; align-items:flex-start; justify-content:space-between; gap:12px; transition:border-color 0.15s; }
.dlc-block-card:hover { border-color:var(--dsw-alias-label-secondary, #52525b); }
.dlc-block-badge { padding:1px 6px; border-radius:4px; font-size:10px; font-weight:600; background:rgba(59,130,246,0.1); color:#60a5fa; border:1px solid rgba(59,130,246,0.2); }
.dlc-btn-reorder-active { background:rgba(245,158,11,0.15) !important; border-color:#f59e0b !important; color:#fbbf24 !important; }
.dlc-btn-code-active { background:rgba(99,102,241,0.2) !important; border-color:#6366f1 !important; color:#a5b4fc !important; }
...
`;
    ...
    const CHAT_CARD_CSS = `
      .dlc-chat-card {
        ...
        gap: 10px;
        padding: 12px;
        border: 1px solid var(--dsw-alias-border-l1, #27272a);
        border-radius: 12px;
        background: var(--dsw-alias-bg-layer-1, #121216);
        max-width: 580px;
        width: 100%;
      ...
      .dlc-chat-card-btn-secondary {
        ...
        padding: 6px 12px;
        border-radius: 8px;
        background: var(--dsw-alias-bg-layer-2, #1f1f27);
    ...
    function registerNativeSidebar(ctx) {
      const tabsService = ctx.sidebarRightTabs;
      const slotsService = ctx.slots;
      if (!tabsService || typeof tabsService.register !== 'function') return () => {};

      const disposers = [];

      try {
        const unregDef = tabsService.register({
          id: '@goodandready/dsh-live-canvas',
          kind: 'live-canvas',
          patterns: [
          ...
          title: (addr) => {
            ...
            return raw || 'Live Canvas';
          },
          guide: [{
            order: 25,
            title: () => (ctx.locale && ctx.locale.getSnapshot?.().active?.startsWith('ru') ? 'Live Canvas' : 'Live Canvas'),
            description: () => (ctx.locale && ctx.locale.getSnapshot?.().active?.startsWith('ru')
      ...
      if (slotsService && typeof slotsService.register === 'function') {
        try {
          const unregSlot = slotsService.register({
            name: 'sidebar.right.pane.tab',
            key: '@goodandready/dsh-live-canvas',
            locale: NS,
            inject: () => ({ ctx })
          }, NativeLiveCanvasTab);
          if (typeof unregSlot === 'function') disposers.push(unregSlot);
        } catch (err) {
          console.warn('[dsh-live-canvas] Error registering native sidebar tab slot:', err);
        }
      }
    ...
    module.exports.apply = function apply(ctx) {
      ...
      // Native DSH 0.1.5-alpha.1 Right Sidebar Integration via ctx.inject
      if (typeof ctx.inject === 'function') {
        try {
          ctx.inject(['sidebarRightTabs'], (sctx) => {
            return registerNativeSidebar(sctx);
          });
        } catch (err) {
          console.warn('[dsh-live-canvas] Error in sidebarRightTabs injection:', err);
        }
      }
