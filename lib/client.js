...
// Provides settings card in "Settings -> Plugins -> Plugin Settings", Live Canvas interactive preview container with telemetry console, annotations, props controls, visual diffs, device matrix, mock data, 1-click Vite packager, and native dsh-better-sidebar integration.
...
`;
    ...
    const i18n = {
      en: {
        ...
        description: 'Interactive in-browser canvas for real-time preview of HTML (with Tailwind CSS & Lucide icons), React components, SVGs, and diagrams with SSE hot-reload, props controls, visual diffs, device matrix, AI mock data, 1-click Vite project packager, and Better Sidebar integration.',
      ...
      ru: {
        title: 'Live Canvas Preview',
        description: 'Интерактивный холст для предпросмотра HTML (с Tailwind CSS и Lucide иконками), React-компонентов, SVG и диаграмм в реальном времени с поддержкой SSE hot-reload, контролов пропсов, визуального сравнения, мульти-девайс матрицы, моковых API, 1-клик упаковщика Vite проектов и нативной интеграции с Better Sidebar.',
    ...
    function LiveCanvasWorkspace(props) {
      ...
      React.useEffect(() => {
        fetch(`/dsh-live-canvas/api/controls?canvasId=${canvasId}`)
          .then(r => r.json())
          .then(data => {
        ...
        fetch(`/dsh-live-canvas/api/mock?canvasId=${canvasId}`)
          .then(r => r.json())
          .then(data => {
      ...
      const handleControlChange = (key, value) => {
        ...
        fetch('/dsh-live-canvas/api/controls', {
          ...
          body: JSON.stringify({ canvasId, values: { [key]: value } })
        }).catch(() => {});
      };
      ...
      const handleExport = () => {
        window.open(`/dsh-live-canvas/api/export/${canvasId}`, '_blank');
      };

      const handlePack = () => {
        window.open(`/dsh-live-canvas/api/pack/${canvasId}?framework=vite-react`, '_blank');
      };

      const handleCompare = () => {
        window.open(`/dsh-live-canvas/diff/${canvasId}`, '_blank');
      };
    ...
    function registerBetterSidebar(ctx) {
      const service = (ctx.get && ctx.get('betterSidebar')) || ctx.betterSidebar;
    ...
    module.exports.apply = function apply(ctx) {
      ...
      // Native Better Sidebar Registration (immediate or on injection)
