...
// Provides settings card in "Settings -> Plugins -> Plugin Settings", Live Canvas interactive preview container with session dropdown switcher, SSE auto-switch, telemetry console, annotations, props controls, visual diffs, device matrix, mock data, 1-click Vite packager, and native dsh-better-sidebar integration.
...
.dlc-panel-container { display:flex; flex-direction:column; width:100%; height:100%; min-height:360px; background:var(--dsw-alias-bg-layer-2, #18181b); border-radius:8px; overflow:hidden; }
...
.dlc-session-dropdown { height:28px; border:1px solid var(--dsw-alias-border-l2, #3f3f46); background:var(--dsw-alias-bg-layer-4, #18181b); color:var(--dsw-alias-label-primary, #ffffff); border-radius:6px; padding:0 8px; font-size:12px; font-weight:600; max-width:240px; cursor:pointer; text-overflow:ellipsis; }
...
`;
    ...
    const i18n = {
      en: {
        ...
        description: 'Interactive in-browser canvas for real-time preview of HTML (with Tailwind CSS & Lucide icons), React components, SVGs, and diagrams with SSE hot-reload, session switcher, props controls, visual diffs, device matrix, AI mock data, 1-click Vite project packager, and Better Sidebar integration.',
        ...
        packBtn: 'Pack Vite Project',
        annotateBtn: 'Annotate',
        controlsBtn: 'Controls',
        compareBtn: 'Compare',
        matrixBtn: 'Matrix',
        mockBadge: 'Mock API Active',
        sidebarTab: 'Live Canvas'
      },
      ru: {
        title: 'Live Canvas Preview',
        description: 'Интерактивный холст для предпросмотра HTML (с Tailwind CSS и Lucide иконками), React-компонентов, SVG и диаграмм в реальном времени с поддержкой переключателя сессий, SSE hot-reload, контролов пропсов, визуального сравнения, мульти-девайс матрицы, моковых API, 1-клик упаковщика Vite проектов и нативной интеграции с Better Sidebar.',
        ...
        packBtn: 'Упаковать в Vite',
        annotateBtn: 'Аннотации',
        controlsBtn: 'Контролы',
        compareBtn: 'Сравнить',
        matrixBtn: 'Матрица',
        mockBadge: 'Мок API активен',
        sidebarTab: 'Live Canvas'
      }
    };
    ...
    function LiveCanvasWorkspace(props) {
        ...
        fetch('/dsh-live-canvas/api/sessions')
          ...
          .then(data => {
            if (data.sessions && Array.isArray(data.sessions)) {
              ...
              if (!canvasId && data.sessions.length > 0) {
      ...
      React.useEffect(() => {
        if (initialCanvasId && initialCanvasId !== 'default') {
      ...
      return React.createElement('div', { className: 'dlc-panel-container' },
        ...
        React.createElement('div', { className: 'dlc-toolbar' },
          React.createElement('div', { className: 'dlc-toolbar-group' },
            ...
            sessions.length > 0 && React.createElement('select', {
              className: 'dlc-session-dropdown',
              value: activeId,
              title: 'Переключить проект / сессию Live Canvas',
          ...
          React.createElement('div', { className: 'dlc-toolbar-group' },
            ...
            }, annotateActive ? '🖍 Drawing...' : (annotations.length > 0 ? `🖍 ${annotations.length} notes` : '🖍 Annotate')),
            React.createElement('button', {
              className: 'dlc-btn',
        ...
        // Viewport (Standard or Matrix)
        viewport === 'matrix' ?
          React.createElement('div', { className: 'dlc-matrix-viewport' },
            // Mobile (375px)
            React.createElement('div', { className: 'dlc-matrix-card', style: { width: '375px' } },
              React.createElement('div', { className: 'dlc-matrix-card-head' },
                React.createElement('span', null, '📱 Mobile (375px)'),
                React.createElement('span', { style: { color: '#a1a1aa' } }, 'Sync Scroll')
              ),
            ...
            React.createElement('div', { className: 'dlc-matrix-card', style: { width: '768px' } },
              React.createElement('div', { className: 'dlc-matrix-card-head' },
                React.createElement('span', null, '📟 Tablet (768px)'),
                React.createElement('span', { style: { color: '#a1a1aa' } }, 'Sync Scroll')
              ),
              React.createElement('iframe', {
            ...
            React.createElement('div', { className: 'dlc-matrix-card', style: { width: '1024px' } },
              React.createElement('div', { className: 'dlc-matrix-card-head' },
                React.createElement('span', null, '💻 Desktop (1024px+)'),
                React.createElement('span', { style: { color: '#a1a1aa' } }, 'Sync Scroll')
              ),
              React.createElement('iframe', {
        ...
        showControls && React.createElement('div', { className: 'dlc-controls-drawer' },
          !controlsSchema || Object.keys(controlsSchema).length === 0 ?
            React.createElement('div', { style: { color: '#71717a' } }, 'No interactive props controls declared for this component.') :
      ...
      try {
        const unregTab = service.registerTab({
          ...
          component: (props) => {
            ...
            return React.createElement(LiveCanvasWorkspace, { canvasId: activeCanvasId });
          }
        });
      ...
      try {
        if (typeof service.registerFileViewer === 'function') {
          const unregViewer = service.registerFileViewer({
            ...
            component: (props) => {
              return React.createElement(LiveCanvasFileViewer, { path: props.path, content: props.content });
            }
          });
