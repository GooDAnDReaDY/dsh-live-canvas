...
`;
    ...
    function parseToolBlockData(block) {
      ...
      try {
        ...
        if (block && block.call && block.call.argsRaw) {
    ...
    function LiveCanvasChatCard(props) {
      ...
      const handleOpenStudio = () => {
        if (typeof window !== 'undefined') {
          ...
          try {
            const sidebar = (ctx && ctx.get && ctx.get('betterSidebar')) || window.__dshBetterSidebar;
            if (sidebar && typeof sidebar.openTab === 'function') {
              sidebar.openTab({ type: 'live-canvas', title: 'Live Canvas' });
            }
          } catch {}
        }
      };
