...
// Provides settings card in "Settings -> Plugins -> Plugin Settings", Live Canvas interactive preview container with workspace file discovery, session switcher, file picker drawer, SSE hot-reload, telemetry console, annotations, props controls, visual diffs, device matrix, mock data, 1-click Vite packager, and native dsh-better-sidebar integration.
...
.dlc-session-dropdown { height:28px; border:1px solid var(--dsw-alias-border-l2, #3f3f46); background:var(--dsw-alias-bg-layer-4, #18181b); color:var(--dsw-alias-label-primary, #ffffff); border-radius:6px; padding:0 8px; font-size:12px; font-weight:600; max-width:260px; cursor:pointer; text-overflow:ellipsis; }
    ...
    function LiveCanvasWorkspace(props) {
      ...
      const openFile = (filePath) => {
        ...
        fetch('/dsh-live-canvas/api/open-file', {
          ...
          .catch(err => {
            console.warn('[LiveCanvas] Failed to open target file -', err);
          });
      };
        ...
        React.createElement('div', { className: 'dlc-toolbar' },
          React.createElement('div', { className: 'dlc-toolbar-group' },
            ...
            React.createElement('select', {
              ...
              value: activeId,
              title: 'Переключить файл рабочей области или сессию Live Canvas',
              onChange: (e) => handleDropdownChange(e.target.value)
            },
              sessions.length > 0 && React.createElement('optgroup', { label: '⚡ Активные сессии' },
                sessions.map(s => {
              ...
              workspaceFiles.length > 0 && React.createElement('optgroup', { label: '📂 Файлы рабочей области' },
            ...
            React.createElement('button', {
              ...
              title: 'Открыть проводник файлов проекта',
              onClick: () => setShowFilePicker(!showFilePicker)
            }, '📂 ' + (workspaceFiles.length > 0 ? `${workspaceFiles.length} файлов` : 'Обзор')),
            React.createElement('button', {
        ...
        showFilePicker && React.createElement('div', { className: 'dlc-picker-drawer' },
          React.createElement('div', { className: 'dlc-picker-head' },
            React.createElement('input', {
              ...
              placeholder: '🔍 Поиск файла в папке проекта (например: coffee-shop-landing/preview.html)...',
              value: fileFilter,
          ...
          filteredFiles.length === 0 ?
            React.createElement('div', { style: { color: '#71717a', padding: '12px 0' } }, 'Файлы интерфейсов (.html, .jsx, .tsx, .svg, .md) не найдены по запросу.') :
