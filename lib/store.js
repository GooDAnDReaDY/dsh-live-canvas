...
export class PreviewStore {
      ...
      if (fs.existsSync(this.persistencePath)) {
        ...
        if (Array.isArray(data.sessions)) {
          for (const s of data.sessions) {
            if (!s || !s.id) continue;
            // Discard session if its bound filePath has been deleted from disk
            if (s.filePath && !fs.existsSync(s.filePath)) {
              continue;
            }
            if (s.filePath && fs.existsSync(s.filePath)) {
              try {
                const live = fs.readFileSync(s.filePath, 'utf8');
                if (live) s.content = live;
              } catch (err) {
                /* ignoreOptionalFailure: keep saved session content */
              }
            }
        ...
        if (Array.isArray(data.snapshots)) {
          for (const [id, list] of data.snapshots) {
            if (id && Array.isArray(list)) this.snapshots.set(id, list);
          }
        }
      }
  ...
  flush() {
    ...
    try {
      ...
      fs.writeFileSync(this.persistencePath, payload, 'utf8');
    } catch (err) { /* ignoreOptionalFailure: bestEffort fallback */ }
  }
  ...
  createOrUpdateSession(data = {}) {
    ...
    const rawContent = typeof data.content === 'string' ? data.content : (existing?.content || '');
    const rawFilePath = data.filePath !== undefined ? data.filePath : (existing?.filePath || null);

    const session = {
      id,
      title: data.title || existing?.title || 'Live Preview',
      content: rawContent,
      componentType: data.componentType || existing?.componentType || autoDetectType(rawContent, rawFilePath || ''),
  ...
  getSession(id) {
    ...
    if (session.filePath && fs.existsSync(session.filePath)) {
      try {
        const live = fs.readFileSync(session.filePath, 'utf8');
        if (live && live !== session.content) {
          session.content = live;
        }
      } catch (err) {
        /* ignoreOptionalFailure: fallback to memory content */
      }
    }
  ...
  getLatestSession() {
    ...
    // Returns the most recently updated/accessed session (last in Map iteration)
  ...
  resolveAnnotation(canvasId, annotationId, note = '') {
    const list = this.annotations.get(canvasId) || [];
    const item = list.find(a => a.id === annotationId);
    if (item) {
      item.status = 'resolved';
      item.resolutionNote = note || 'Resolved by agent';
