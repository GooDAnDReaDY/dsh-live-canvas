...
import { sanitizePath, resolveSafePath } from './security.js';
...
export class WorkspaceWatcher {
  constructor(store, eventHub, options = {}) {
    ...
    const rawRoots = options.workspaceRoots || [options.workspaceDir || process.cwd()];
    this.workspaceRoots = (Array.isArray(rawRoots) ? rawRoots : [rawRoots]).filter(Boolean).map(r => path.resolve(r));
    if (this.workspaceRoots.length === 0) {
      this.workspaceRoots.push(process.cwd());
    }
    this.workspaceDir = this.workspaceRoots[0];
    this.debounceMs = options.debounceMs || 150;
    this.watchers = new Map(); // canvasId -> { watcher, filePath, absPath, clearTimer }
    this.filesCacheTtlMs = options.filesCacheTtlMs || 3000;
  ...
  listWorkspaceFiles(subDir = '', maxDepth = 5, maxFiles = 500) {
    const cacheKey = `${subDir}::${maxDepth}::${maxFiles}`;
    const cached = this.filesCache.get(cacheKey);
    const now = Date.now();
    if (cached && (now - cached.timestamp) < this.filesCacheTtlMs) {
      return cached.files;
    }
    ...
    function walk(currentDir, currentRel, depth) {
      ...
      try {
        ...
        /* intentional: unreadable directory skipped */
        return;
      }
        ...
        if (entry.isDirectory()) {
          ...
          if (PREVIEWABLE_EXTS.has(ext)) {
            try {
              ...
              /* intentional: inaccessible file stat skipped */
            }
          }
        }
      }
    ...
    try {
      const targetDir = subDir ? resolveSafePath(subDir, this.workspaceRoots) : base;
    ...
    this.filesCache.set(cacheKey, { timestamp: now, files: sorted });
    return sorted;
  }

  openWorkspaceFile(relativePath) {
    if (!relativePath) throw new Error('relativePath is required');
    const absPath = resolveSafePath(relativePath, this.workspaceRoots);
    if (!fs.existsSync(absPath)) {
      const err = new Error(`File not found: ${relativePath}`);
      err.code = 'ENOENT';
      err.targetPath = relativePath;
      err.allowedRoots = this.workspaceRoots;
      throw err;
    }
  ...
  watchFile(canvasId, relativePath) {
    ...
    let absPath;
    try {
      absPath = resolveSafePath(relativePath, this.workspaceRoots);
    } catch (err) {
      console.warn('[WorkspaceWatcher] Cannot watch file outside roots:', relativePath, err.message);
      this.eventHub.broadcast('watcher_error', {
        canvasId,
        filePath: relativePath,
        error: `Cannot watch file outside allowed roots: ${err.message}`
      });
      return false;
    }
    ...
    const onFileEvent = (eventType) => {
      this.invalidateFilesCache();
      if (eventType === 'rename') {
        setTimeout(() => {
          if (!this.watchers.has(canvasId)) return;
          if (fs.existsSync(absPath)) {
            try {
              if (currentWatcher) currentWatcher.close();
            } catch (err) {
              /* intentional: closing replaced watcher handle */
            }
            try {
              currentWatcher = fs.watch(absPath, onFileEvent);
              const rec = this.watchers.get(canvasId);
              if (rec) rec.watcher = currentWatcher;
            } catch (err) {
              console.warn(`[WorkspaceWatcher] Failed to re-attach watch on ${absPath}:`, err);
              this.eventHub.broadcast('watcher_error', {
                canvasId,
                filePath: relativePath,
                error: `Watcher failed to re-attach: ${err.message}`
              });
            }
          }
        }, 50);
    ...
    try {
      ...
      console.warn(`[WorkspaceWatcher] Failed to start watcher on ${absPath}:`, err);
      this.eventHub.broadcast('watcher_error', {
        canvasId,
        filePath: relativePath,
        error: `Watcher startup failed: ${err.message}`
      });
      return false;
    }
  }
  ...
  unwatch(canvasId) {
    if (this.watchers.has(canvasId)) {
      ...
      try {
        ...
        /* intentional: closing already released watcher handle */
      }
