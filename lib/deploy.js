...
export function buildDeploymentBundle(input = {}) {
  const { session, target = 'vercel', options = {} } = (input && typeof input === 'object') ? input : {};
  ...
  const cleanName = (title || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
  const safeName = cleanName || `canvas-${session?.id || 'app'}`;
