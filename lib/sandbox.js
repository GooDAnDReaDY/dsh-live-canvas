...
export function getSandboxHeaders(options = {}) {
  ...
  if (options.allowOrigin && options.allowOrigin !== '*') {
    headers['Access-Control-Allow-Origin'] = options.allowOrigin;
  }
  return headers;
}

export { sanitizePath, resolveSafePath, isTrustedRequest, isLoopbackAddress } from './security.js';
...
(function() {
  ...
  // --- Console & Runtime Telemetry Bridge ---
  const origLog = console.log;
  const origWarn = console.warn;
  const origError = console.error;
  const origInfo = console.info;

  function postConsoleMsg(level, args) {
    try {
    ...
    } catch (err) { /* ignoreOptionalFailure: bestEffort fallback */ }
  }

  console.log = function(...args) { origLog.apply(console, args); postConsoleMsg('log', args); };
  console.warn = function(...args) { origWarn.apply(console, args); postConsoleMsg('warn', args); };
  console.error = function(...args) { origError.apply(console, args); postConsoleMsg('error', args); };
  console.info = function(...args) { (origInfo || origLog).apply(console, args); postConsoleMsg('info', args); };

  window.addEventListener('error', function(event) {
    postConsoleMsg('error', [event.message || 'Uncaught Error', (event.filename ? '@ ' + event.filename + ':' + event.lineno : '')]);
  });
  window.addEventListener('unhandledrejection', function(event) {
    postConsoleMsg('error', ['Unhandled Promise Rejection:', event.reason ? (event.reason.message || String(event.reason)) : 'Unknown rejection']);
  });

  window.addEventListener('message', function(ev) {
    if (!ev.data) return;
    if (ev.data.type === 'DLC_SET_STATE_PRESET') {
      const preset = ev.data.preset;
      document.documentElement.setAttribute('data-state-preset', preset);
      window.dispatchEvent(new CustomEvent('dlc-state-preset', { detail: { preset } }));
      postConsoleMsg('info', ['[LiveCanvas] State preset switched to:', preset]);
    } else if (ev.data.type === 'DLC_REQUEST_SNAPSHOT') {
      const html = document.documentElement.outerHTML;
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({
          type: 'DLC_SNAPSHOT_RESULT',
          canvasId: CANVAS_ID,
          html,
  ...
  overlayStyle.textContent = \`
    ...
    @keyframes dlcGlowPulse {
      ...
      50% { box-shadow: 0 0 30px rgba(99, 102, 241, 0.55); }
    }
    .dlc-motion-fade-up {
      animation: dlcFadeUp 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }
    .dlc-motion-glow {
      animation: dlcGlowPulse 2.5s ease-in-out infinite;
    }
    .dlc-motion-tilt {
      transition: transform 0.25s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.25s;
    }
    .dlc-motion-tilt:hover {
      transform: perspective(600px) rotateX(3deg) rotateY(-3deg) translateY(-3px);
  ...
  function renderStyleTweaker(target) {
    ...
    tweakerBarElem.innerHTML = \`
      ...
      <div class="dlc-tweaker-row">
        <span class="dlc-tweaker-label">Text</span>
        <span class="dlc-tweaker-chip" data-cls="text-white" data-re="^text-(white|slate|amber|emerald|blue|rose|violet)">White</span>
        <span class="dlc-tweaker-chip" data-cls="text-amber-400" data-re="^text-(white|slate|amber|emerald|blue|rose|violet)">Amber</span>
        <span class="dlc-tweaker-chip" data-cls="text-emerald-400" data-re="^text-(white|slate|amber|emerald|blue|rose|violet)">Emerald</span>
        <span class="dlc-tweaker-chip" data-cls="text-blue-400" data-re="^text-(white|slate|amber|emerald|blue|rose|violet)">Blue</span>
        <span class="dlc-tweaker-chip" data-cls="text-rose-400" data-re="^text-(white|slate|amber|emerald|blue|rose|violet)">Rose</span>
      </div>
      <div class="dlc-tweaker-row">
        <span class="dlc-tweaker-label">Padding</span>
      ...
      <div class="dlc-tweaker-row">
        <span class="dlc-tweaker-label">Radius</span>
        <span class="dlc-tweaker-chip" data-cls="rounded-none" data-re="^rounded">Sharp</span>
        ...
        <span class="dlc-tweaker-chip" data-cls="rounded-full" data-re="^rounded">Pill</span>
        <span class="dlc-tweaker-chip" data-cls="shadow-lg" data-re="^shadow">Shadow</span>
      </div>
    \`;
  ...
  document.addEventListener('dblclick', (e) => {
    ...
    if (!badgeElem) {
      ...
      badgeElem.innerHTML = '<span>✏️</span> <span>Text Edit Mode: <b>Enter</b> to save, <b>Esc</b> to cancel</span>';
