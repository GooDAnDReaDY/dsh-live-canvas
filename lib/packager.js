...
export function buildStandaloneHtmlBundle(session, options = {}) {
  const title = session.title || 'Live Canvas Standalone Export';
  const rawContent = session.content || session.code || '<div class="p-6">Empty Component</div>';
  const theme = session.theme || 'dark';
  const isDark = theme === 'dark' || !theme.includes('light');
