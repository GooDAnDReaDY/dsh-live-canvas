...
import { sanitizePath, resolveSafePath } from './sandbox.js';
...
export function registerLiveCanvasTools(ctx, store, eventHub, options = {}) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Render or update interactive live canvas preview for HTML, React JSX, SVG, Mermaid, or Markdown.',
    parameters: {
      content: {
        type: 'string',
        description: 'Code, markup, or markdown content to render.'
      },
      filePath: {
        type: 'string',
        description: 'Optional relative file path in workspace.'
      },
      title: {
        type: 'string',
        description: 'Title for preview session (e.g. "Hero Component").'
      },
      componentType: {
        type: 'string',
        enum: ['html', 'react', 'svg', 'mermaid', 'markdown', 'gallery', 'auto'],
        description: 'Component type. Default: "auto".'
      },
      viewport: {
        type: 'string',
        enum: ['responsive', 'mobile', 'tablet', 'desktop', 'matrix'],
        description: 'Target viewport layout.'
      },
      theme: {
        type: 'string',
      ...
      canvasId: {
        type: 'string',
        description: 'Optional canvas ID to update existing session.'
      },
      controls: {
        type: 'object',
    ...
    execute: async (args = {}) => {
      ...
      if (!content && filePath) {
        try {
          ...
          const targetPath = resolveSafePath(filePath, roots);
          if (fs.existsSync(targetPath)) {
            content = fs.readFileSync(targetPath, 'utf8');
          } else {
            return {
              error: `Target not found at path: ${filePath}`,
              code: 'ENOENT',
              allowedRoots: roots,
              success: false
            };
          ...
          return {
            ...
            code: err.code || 'ERR_SAFE_PATH_FAILED',
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args = {}) => {
      ...
      if (args.destinationPath) {
        try {
          const target = resolveSafePath(args.destinationPath, getWorkspaceRoots());
