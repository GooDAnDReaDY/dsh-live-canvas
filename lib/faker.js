...
const FIRST_NAMES = ['Alex', 'Emma', 'David', 'Sophia', 'Lucas', 'Olivia', 'Liam', 'Mia', 'Noah', 'Ava', 'Ethan', 'Isabella', 'Mason', 'Charlotte', 'James', 'Harper'];
const LAST_NAMES = ['Miller', 'Johnson', 'Smith', 'Williams', 'Brown', 'Davis', 'Wilson', 'Anderson', 'Taylor', 'Thomas', 'Moore', 'Jackson', 'Martin', 'Lee'];
...
export function generateProducts(count = 6) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const price = randomNumber(19, 299);
...
export function generateAnalytics(days = 7) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    points.push({
      ...
      revenue: randomNumber(4500, 21000)
    });
  }
  return points;
}
