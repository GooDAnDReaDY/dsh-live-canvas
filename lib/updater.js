...
/**
 * Host-side one-click updater for @goodandready/dsh-live-canvas.
...
export function registerPluginUpdater(ctx, options) {
  ...
  if (typeof ctx?.webServer?.register !== 'function') {
    ctx?.logger?.warn?.('[dsh-live-canvas] webServer service unavailable, updater skipped');
    return () => {};
  }

  return ctx.webServer.register({
    kind: 'exact',
    path: options.endpoint || '/api/dsh-live-canvas/update',
    handler: async (request, response) => {
      try {
        ...
        ctx?.logger?.warn?.(`[dsh-live-canvas] plugin updater failed: ${String(error)}`);
