...
export function parseDesignTokens(tokensInput, options = {}) {
  let tokens = {};
  if (typeof tokensInput === 'string') {
    try {
      tokens = JSON.parse(tokensInput);
    } catch (e) {
      throw new Error('Invalid JSON format for design tokens');
    }
  } else if (typeof tokensInput === 'object' && tokensInput !== null) {
    tokens = tokensInput;
  } else {
    throw new Error('Design tokens input must be an object or valid JSON string');
  }
  ...
  return {
    ...
    cssRule: `:root {\n${cssVariables.join('\n')}\n}`,
    variables: cssVariables.map(line => line.trim())
  };
}
