// dsh-live-canvas: dynamic transpiler, HTML wrapper, Component Gallery, Visual Diff, and Multi-Device Matrix generator.
// Handles HTML, React JSX (Babel standalone with dynamic props controls), SVG, Mermaid, Markdown, Component Galleries, Before/After Split Diffs, and Multi-Device Matrix views.
...
export function buildDiffWrapper(currentSession, previousSession, options = {}) {
  const title = options.title || 'Visual Diff: Before vs After';
  const theme = options.theme || 'dark';

  const beforeHtml = transpileAndWrap(previousSession || { content: '<p>No previous version</p>' });
  const afterHtml = transpileAndWrap(currentSession || { content: '<p>No current version</p>' });
