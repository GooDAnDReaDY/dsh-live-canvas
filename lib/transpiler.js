      ...
      if (cssSpec.startsWith('.')) {
        ...
        if (resolvedCss && !visited.has(resolvedCss)) {
          ...
          try {
            ...
            /* ignoreOptionalFailure: optional component css file not readable */
          }
        }
      }
      return '';
...
<body>
  ...
  <script>
    window.addEventListener('error', function(e) {
      var box = document.getElementById('dlc-error-boundary');
      if (box) {
        box.style.display = 'block';
        box.textContent = 'Syntax / Compile Error: ' + (e.error ? (e.error.stack || e.error.message) : (e.message || String(e)));
      }
    });
  </script>
...
export function buildDiffWrapper(sessionA, sessionB) {
  sessionA = (sessionA && typeof sessionA === 'object') ? sessionA : { title: 'Baseline', id: 'base', content: '' };
  sessionB = (sessionB && typeof sessionB === 'object') ? sessionB : { title: 'Revision', id: 'rev', content: '' };
...
export function buildMatrixWrapper(session) {
  session = (session && typeof session === 'object') ? session : { title: 'Live Preview', id: 'default', content: '' };
...
<body>
  <div class="matrix-header">
    <span>📱 Synchronized Multi-Device Matrix</span>
    <span style="font-weight:400; font-size:12px; color:#a1a1aa;">Scroll is automatically synchronized across all viewports</span>
  </div>
...
<body>
  <div class="gallery-head">
    ...
    <span style="font-size:12px; color:#3b82f6; font-weight:600;">${variants.length} variants</span>
  </div>
...
export function getTranspileCacheSize() {
  return transpileCache.size;
}
...
function getSessionCacheKey(session) {
  ...
  const type = session.componentType || 'html';
  const title = session.title || '';
  const filePath = session.filePath || '';
  const cv = session.controlValues ? JSON.stringify(session.controlValues) : '';
  const vars = Array.isArray(session.variants) ? session.variants.length : 0;
  const raw = session.content || '';
  const contentSig = raw.length <= 128
    ? raw
    : `${raw.length}:${raw.slice(0, 64)}:${raw.slice(-64)}`;
  return `${id}::${updated}::${type}::${theme}::${title}::${filePath}::${cv}::${vars}::${contentSig}`;
}
...
export function transpileAndWrap(session) {
  ...
  if (transpileCache.has(cacheKey)) {
    ...
    transpileCache.delete(cacheKey);
    transpileCache.set(cacheKey, cached);
    return cached;
  }
  ...
  if (componentType === 'gallery' || (Array.isArray(variants) && variants.length > 0)) {
    result = buildGalleryWrapper(session);
  } else {
    const effectiveType = componentType === 'gallery' ? 'react' : componentType;
    switch (effectiveType) {
      case 'react':
        result = buildReactWrapper(content, { title, theme, customCss, customJs, controlValues, filePath, workspaceDir });
        break;
      case 'svg':
        result = buildSvgWrapper(content, { title, theme });
        break;
      case 'mermaid':
        result = buildMermaidWrapper(content, { title, theme });
        break;
      case 'markdown':
        result = buildMarkdownWrapper(content, { title, theme });
        break;
      case 'html':
      default:
        result = buildHtmlWrapper(content, { title, theme, customCss, customJs, filePath, workspaceDir });
        break;
    }
  }
