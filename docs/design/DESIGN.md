...
- **Архитектурное обоснование серверных модулей (`lib/index.js`, `lib/transpiler.js`, `lib/sandbox.js`)**:
  - **`lib/index.js` (1028 строк) — Точка входа Cordis и диспетчер WebServer-маршрутов**:
    - Модуль реализует центральную точку подключения плагина (`apply(ctx, config)`). Внутри него атомарно регистрируются 14 связанных эндпоинтов Web-сервера DSH (`/sandbox`, `/events`, `/api/open-file`, `/api/save-content`, `/api/save-classes`, `/api/save-reorder`, `/api/diff`, `/api/matrix`, `/api/mock`, `/api/pack`, `/api/export`, `/api/templates`, `/api/dsh-live-canvas/update`, etc.) и хук выгрузки.
    - Все маршруты совместно используют единое замыкание контекста плагина (`PreviewStore`, `EventHub`, `WorkspaceWatcher` и `workspaceRoots`). Дробление файла на множество изолированных фабрик обработчиков привело бы к размазыванию логики управления жизненным циклом и усложнению сквозной трассировки зависимостей между SSE-шиной и REST-маршрутами без функционального выигрыша.
  - **`lib/transpiler.js` (934 строки) — Автономный in-memory компилятор и генератор разметки**:
    - Модуль выполняет мгновенную сборку и трансляцию кода компонентов в готовый HTML-документ за <10 мс без привлечения внешних тяжеловесных бандлеров.
    - Объединяет эвристический детектор типов (`autoDetectType`) и шаблонизаторы для всех 6 поддерживаемых форматов (React 18 Babel Standalone + Lucide Icons, Vue 3, HTML5+Tailwind, SVG, Mermaid, Markdown). Сохранение модуля единым гарантирует синхронное связывание шаблонов с автодетектом типов и исключает циклические зависимости между шаблонами разметки и системными темами оформления.
  - **`lib/sandbox.js` (831 строка) — Изолированный рантайм для iframe-песочницы**:
    - Модуль формирует заголовки жесткой изоляции (`getSandboxHeaders`: CSP `default-src 'self' ...`, `X-Frame-Options: SAMEORIGIN`) и инжектирует в тело документа автономный клиентский скрипт `injectSandboxRuntime`.
    - Скрипт рантайма должен исполняться внутри изолированного sandboxed `<iframe>` как единая самодостаточная строка-бандл: он реализует трекинг кликов инспектора DOM, SSE-клиент автообновления (`/events`), перехват горячих клавиш (<kbd>F5</kbd>, <kbd>Ctrl+R</kbd>, <kbd>Escape</kbd>), инлайн-редактор стилей Tailwind и postMessage-мост. Внутри защищённой песочницы с политикой `SAMEORIGIN` запрещена асинхронная подгрузка внешних фрагментированных модулей, поэтому целостность строки-рантайма является обязательным требованием безопасности.
- **Персистентность сессий при рестарте харнесса (Refs: #123)**:
  - Метод `_loadFromDisk` при инициализации хранилища восстанавливает карту активных сессий, отфильтровывая сессии с исчезнувшими файлами рабочей области и подтягивая свежий контент с диска.
  - При остановке/выгрузке плагина в хуке завершения вызывается `store.flush()` для гарантированного синхронного сброса незаписанных сессий на диск. Метод `store.clear()` по умолчанию не затирает дисковый файл `sessions.json`, что предотвращает потерю открытых превью при штатных перезапусках службы `dsh-web`.
